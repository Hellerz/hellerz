...
!function($){
    ...
    MMGrid.prototype = {
        ...
        , _initEvents: function(){
            ...
            document.addEventListener('keydown',function(e){
                if(that.__isGridActive){
                    ...
                    var len=that.rowsLength();
                    var lastindex = index;

                    if(e.which === 65 && e.ctrlKey){//ctrl+a
                        that.select('all');
                        return;
                    }
                    if(e.which === 36){
                        for (index = 0; index < len; index++) {
                            var display = $trs.eq(index).css("display");
                    ...
                    if(e.which === 33){
                        parent.scrollTop(parent.scrollTop()-parent.innerHeight());
                        return;
                    }
                    if(e.which === 34){
                        parent.scrollTop(parent.scrollTop()+parent.innerHeight());
        ...
        , updateRow: function(item, selector){
            ...
            var oldItem = $tr.data('item');

            
            var checked = $tr.find('td:first :checkbox').is(':checked');
            $tr.html(this._rowHtml(item, selector).slice(4,-5));
            if(opts.checkCol){
            ...
            this.$body.triggerHandler('rowUpdated', [oldItem, item, selector,$tr]);
        }
